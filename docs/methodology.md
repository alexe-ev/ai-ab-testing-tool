...
**Multiple dimensions mean multiple tests.** Each rubric dimension gets its own t-test. With 3 dimensions at p < 0.05, the chance that at least one comes out "significant" by luck alone is about 14% (about 23% with 5 dimensions). There is no built-in correction for this. Treat a single borderline p-value among several dimensions as weak evidence: trust results where significance holds together with a non-trivial effect size and the head-to-head count points the same way. If you need a strict threshold, apply a Bonferroni-style correction manually: divide 0.05 by the number of dimensions.

**Judge score stability is not measured.** The swap test catches positional bias in pairwise comparisons, but nothing estimates how stable the pointwise scores are: the same response scored twice can come back with different numbers. Temperature 0 reduces this but does not eliminate it, and rubric ambiguity adds more. Per-dimension scores are noisier than they look; head-to-head win rates and effect sizes aggregate over more judgments and are the more robust signals.
